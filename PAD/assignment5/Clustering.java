...
public class Clustering{
	...
	public void start() throws InterruptedException{
		...
		handleEvents(v.getInputSource(), v, ds.getNumberOfClustersWanted(), cr, clusterMaker, c);
	}
	...
	private View setupView(Dataset ds, ClusterRow cr){
		...
		if(ds.getNumberOfVariables() == 2){
			...
			v = new Cartesian(cr, ds.getDataName(), ds.getVariableName(0), ds.getVariableName(1));
			System.out.println("DENDROGRAM NOT DONE");
			System.exit(1);
		}	
		return v;
	}
	...
	private void handleEvents(DrawUserInterface inputSource, View v, int targetClusters, ClusterRow cr, Clusterer clusterMaker, ClusterMethod c) throws InterruptedException{
		...
		while(true){
			...
			switch(userInput.data){
				case("a"): 
						if(cr.getNumberOfClusters() > targetClusters){
