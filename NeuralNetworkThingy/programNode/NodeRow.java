...
public class NodeRow {
	...
	public int getNumberOfPoints(){
		int total = 0;
		for(int i = 0; i < numberOfElements; i++){
			total += row[i].getNumberOfPointsOut();
		}
		return total;
	}
	...
	public NodeRow duplicate(){
		...
		for(int i = 0; i < numberOfElements; i++){
			...
			PointsOutVector[] vecs = row[i].getPointsOutVectors();
			for(int j = 0; j < vecs.length; j++){
				next.pointTo(nodeRow2.getNodeWithName(vecs[j].getToName()), vecs[j].getWeight());
			}
			...
			if(row[i] instanceof Stimnode){
				...
				nextStim.giveSource(vec.getSourceX(),vec.getSourceY());
			}
		}
		return nodeRow2;
	}
	
	public boolean isNodeWithin(double x, double y, double nodeBufferDistance){
		for(int i = 0; i < numberOfElements; i++){
			if(row[i].isWithin(x, y, nodeBufferDistance))
				return true;
		}
		return false;
	}
	...
	public Node getNodeWithName(String queryName){
		for(int i = 0; i < numberOfElements; i++){
